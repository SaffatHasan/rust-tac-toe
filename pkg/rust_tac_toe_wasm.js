...
const WasmGameEngineFinalization = (typeof FinalizationRegistry === 'undefined')
    ? { register: () => {}, unregister: () => {} }
    : new FinalizationRegistry(ptr => wasm.__wbg_wasmgameengine_free(ptr >>> 0, 1));
...
export class WasmGameEngine {
    ...
    get_state() {
        ...
        let deferred1_1;
        try {
            const ret = wasm.wasmgameengine_get_state(this.__wbg_ptr);
            deferred1_0 = ret[0];
            deferred1_1 = ret[1];
            return getStringFromWasm0(ret[0], ret[1]);
        } finally {
     ...
     */
    play_move(position) {
        const ret = wasm.wasmgameengine_play_move(this.__wbg_ptr, position);
        if (ret[1]) {
            throw takeFromExternrefTable0(ret[0]);
        }
    }
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('rust_tac_toe_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
